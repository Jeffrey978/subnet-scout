...
def normalize_subnet(row: dict, identity: dict | None = None) -> dict:
    netuid = _pick(row, "netuid", "net_uid", "id")
    ident = identity or {}
    name = _pick(ident, "subnet_name", "name") or _pick(row, "name", "subnet_name") or ""
    symbol = _pick(ident, "symbol", "ticker") or _pick(row, "symbol") or ""
    ...
    return {
        ...
        "active_miners": _to_int(_pick(row, "active_keys", "active_miners", "neurons", "subnet_size")),
        "active_validators": _to_int(_pick(row, "active_validators", "validators")),
        "max_neurons": _to_int(_pick(row, "max_neurons", "max_n")),
        "price_tao": _to_float(_pick(row, "price", "alpha_price", "price_in_tao")),
        "price_change_24h": _to_float(_pick(row, "price_change_24h", "price_change_1d_percent")),
        "volume_24h": _to_float(_pick(row, "volume_24h", "alpha_volume_24h")),
        "tao_in_pool": _to_float(_pick(row, "tao_in", "tao_in_pool")),
        "raw": row,
    }


def merge(subnets: list[dict], identities: list[dict] | None) -> list[dict]:
    ...
    for row in subnets:
        ...
        if netuid is None:
            continue
        out.append(normalize_subnet(row, ident_by_uid.get(int(netuid))))
